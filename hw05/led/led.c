...
static unsigned int gpioLED1 = 51;           ///< Default GPIO for the LED is 51 for P9_16
module_param(gpioLED1, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioLED1, " GPIO LED1 number (default=51)");     ///< parameter description

static unsigned int gpioLED2 = 50;           ///< Default GPIO for the LED is 50 for P9_14
module_param(gpioLED2, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioLED2, " GPIO LED2 number (default=50)");     ///< parameter description
...
MODULE_PARM_DESC(blinkPeriod1, " LED1 blink period in ms (min=1, default=1000, max=10000)");
...
MODULE_PARM_DESC(blinkPeriod2, " LED2 blink period in ms (min=1, default=1000, max=10000)");
...
static bool ledOn2 = 0;                     ///< Is the second LED on or off?
...
static ssize_t period1_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf){
   return sprintf(buf, "%d\n", blinkPeriod1);
}

/** @brief A callback function to store the LED period value */
static ssize_t period1_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count){
   ...
   if ((period>1)&&(period<=10000)){        // Must be 2ms or greater, 10secs or less
      blinkPeriod1 = period;                 // Within range, assign to blinkPeriod1 variable
   }
   return period;
}
...
static ssize_t period2_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count){
   ...
   sscanf(buf, "%du", &period);             // Read in the period as an unsigned int
   if ((period>1)&&(period<=10000)){        // Must be 2ms or greater, 10secs or less
      blinkPeriod2 = period;                 // Within range, assign to blinkPeriod2 variable
   }
   return period;
}
 ...
 *  The period variable is associated with the blinkPeriod1 variable and it is to be exposed
 *  with mode 0666 using the period1_show and period1_store functions above
 */
static struct kobj_attribute period_attr1 = __ATTR(blinkPeriod1, 0660, period1_show, period1_store);
static struct kobj_attribute period_attr2 = __ATTR(blinkPeriod2, 0660, period2_show, period2_store);
...
static struct attribute *ebb_attrs[] = {
   &period_attr1.attr,                       // The period at which the LED1 flashes
   &period_attr2.attr,                       //Period of LED2 flashes
...
static int flash1(void *arg){
   ...
   printk(KERN_INFO "EBB LED1: Thread has run to completion \n");
   return 0;
}

/** @brief The LED2 Flasher main kthread loop
 *
 *  @param arg A void pointer used in order to pass data to the thread
 *  @return returns 0 if successful
 */
...
static int __init ebbLED_init(void){
   ...
   // add the attributes to /sys/ebb/ -- for example, /sys/ebb/led49/ledOn1
   ...
   ledOn1 = true;
   gpio_request(gpioLED1, "sysfs");          // gpioLED1 is 49 by default, request it
   ...
   task1 = kthread_run(flash1, NULL, "LED_flash_thread");  // Start the LED flashing thread
   task2 = kthread_run(flash2, NULL, "LED_flash_thread");
   if(IS_ERR(task1)){                                     // Kthread name is LED_flash_thread
      printk(KERN_ALERT "EBB LED1: failed to create the task\n");
